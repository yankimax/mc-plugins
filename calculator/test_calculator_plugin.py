...
def _load_plugin_module():
    plugin_path = pathlib.Path(__file__).resolve().parent / 'files' / 'calculator' / 'plugin.py3'
    loader = importlib.machinery.SourceFileLoader('calculator_plugin', str(plugin_path))
    spec = importlib.util.spec_from_loader(loader.name, loader)
    if spec is None:
        raise RuntimeError('failed to create import spec for calculator plugin')
