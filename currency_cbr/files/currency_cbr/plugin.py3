...
CMD_ROUTE_CONVERSION = 'currency:cbr:route-conversion'
CONVERSION_ROUTE_PRIORITY = 100


class CurrencyCbrPlugin(MinaChanPlugin):
    ...
    def on_init(self) -> None:
        self.add_listener('currency:cbr:convert', self.on_convert, listener_id='currency_cbr_convert')
        self.add_listener(
            CMD_ROUTE_CONVERSION,
            self.on_route_conversion,
            listener_id='currency_cbr_route_conversion',
        )

        self.register_command(
            'currency:cbr:convert',
            {
                'en': 'Convert currency by CBR rates',
                'ru': 'Конвертировать валюту по курсам ЦБ РФ',
            },
            {
                'amount': 'Numeric amount, default 1',
                'from': 'Source currency code (USD, RUB, EUR...)',
                'to': 'Target currency code (USD, RUB, EUR...)',
    ...
    def on_route_conversion(self, sender: str, data: Any, tag: str) -> None:
        ...
        if text and contains_nbs_marker(text):
    ...
    def _process(self, parsed: Tuple[float, str, str], say_on_fail: bool) -> None:
        ...
        try:
            ...
            self.log(f'currency_cbr error: {error}')
            if say_on_fail:
    ...
    def _load_rates(self) -> Dict[str, Any]:
        ...
        request = urllib.request.Request(CBR_URL, headers={'User-Agent': 'MinaChan/1.0'})
        ...
        if not isinstance(rates_raw, dict):
            raise ValueError('CBR payload does not have rates map')
        ...
        out = {'date': payload.get('date'), 'rates': rates}
    ...
    def _convert_amount(self, amount: float, source: str, target: str, rates: Dict[str, float]) -> float:
        rub_amount = amount if source == 'RUB' else amount / float(rates[source])
        return rub_amount if target == 'RUB' else rub_amount * float(rates[target])
