...
def _load_plugin_module():
    ...
    if spec is None:
        raise RuntimeError('failed to create import spec for currency_cbr plugin')
...
CurrencyCbrPlugin = _MODULE.CurrencyCbrPlugin


class CurrencyCbrPluginTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plugin = CurrencyCbrPlugin()
    ...
    def test_route_conversion_processes_plain_currency_request(self) -> None:
        ...
        self.assertEqual(self.process_calls, [((15.0, 'USD', 'RUB'), True)])
        self.assertEqual(self.next_alternatives, [])

    def test_route_conversion_skips_nbs_marked_request(self) -> None:
        payload = {'text': 'nbs 15 eur в rsd'}
